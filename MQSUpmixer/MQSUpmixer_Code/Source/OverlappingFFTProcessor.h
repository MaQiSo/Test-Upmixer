 ...
 */
 ...
 /*
  ...
  2021-05-01
     Modified by Qisong Ma:
     - adding processorSampleRate, set from prepare() in PluginProcessor's 
       prepareToPlay()
     - change variable 'fft' to ScopedPointer
  ==============================================================================
  */
...
public:
    ...
    : fft (new dsp::FFT(fftSizeAsPowerOf2)), fftSize (1 << fftSizeAsPowerOf2), hopSize (fftSize >> hopSizeDividerAsPowerOf2)
    {
    ...
    void prepare (const double sampleRate, const int maximumBlockSize, const int numInputChannels, const int numOutputChannels)
    {
        ...
        processorSampleRate = (float)(sampleRate);
    }
...
protected:
    ...
    const int fftSize;
    const int hopSize;
    float processorSampleRate{ 44100.0f }; // default sample rate
